...
file_address = data_folder/'spectra/SHOC579_MANGA38-35.txt'
...
spec.fit.bands('Fe3_4658A', bands_default, cont_source='adjacent')
spec.plot.bands(rest_frame=True)
spec.fit.bands('Fe3_4658A', bands_default, shape='abs', fit_cfg={'Fe3_4658A_center': {'value':4651.0}}, cont_source='adjacent')
spec.plot.bands(rest_frame=True)
...
lime.lineDB.set_database(units_wave='nm', default_shape='abs', update_labels=True, update_latex=True)

bands_nm = spec.retrieve.lines_frame(line_list=['Fe3_465.8nm'], band_vsigma=200)
spec.fit.bands('Fe3_465.8nm', bands=bands_nm, cont_source='adjacent')
spec.plot.bands(rest_frame=True)

spec.fit.bands('Fe3_465.8nm', bands_nm, shape='emi')
spec.plot.bands(rest_frame=True)
spec.fit.bands('Fe3_465.8nm', bands_nm, fit_cfg={'Fe3_4658A_center': {'value':465.1}}, cont_source='adjacent')
spec.plot.bands(rest_frame=True)
...
spec.fit.bands('Fe3_4658A', bands_obj, cont_source='adjacent')
spec.plot.bands(rest_frame=True)

# Using the shape argument does not work now because the table input has preference over the argument
spec.fit.bands('Fe3_4658A', bands_obj, shape='emi', cont_source='adjacent')
spec.plot.bands(rest_frame=True, ax_cfg={'title': 'Lines frame data priority over "shape" argument'})

# To overwrite the shape value on the input tables you need to set it the fit_cfg or use the line label suffix.
fit_cfg = {'transitions': {"Fe3_4658A": {"wavelength": 4658, "shape": "emi"}}}

spec.fit.bands('Fe3_4658A', bands_obj, fit_cfg=fit_cfg, cont_source='adjacent')
spec.plot.bands(rest_frame=True, ax_cfg={'title': 'Fitting configuration priority over lines frame data'})
